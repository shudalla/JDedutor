 ...
 * @author Shu
 */
public class ModusTolens extends IRegras {
        ...
        if (this.tokens.get(this.tokens.size() - 1).getLinha() > 1) {
            ...
            for (TipoToken token : pilha) {
                ...
                if (token.getNome().equals("->")) {
                    ...
                    if (!this.jaContemLinha(this.precedente)) {
    ...
    public void gravaPreConse(int index, Stack<TipoToken> pilha) {
        ...
        //    this.precedente.add(new TipoToken(")", "PARENTESES_FECHA", linhaEliminadaSeta));
        //  this.limpaParenteses(this.precedente);
        if (this.precedente.size() > 3) {
            if (!pilha.get(index + 2).getTipo().equals("ESCOPO")) {

                this.precedente.add(0, new TipoToken("(", "PARENTESES_ABRE", linhaEliminadaSeta));
                this.precedente.add(new TipoToken(")", "PARENTESES_FECHA", linhaEliminadaSeta));

            }

        }
        this.precedente.add(0, new TipoToken("~", "NEGACAO", linhaEliminadaSeta));

        //PARA REMOVER O "->"
        index--;

//        this.consequente.add(new TipoToken("~", "NEGACAO", linhaEliminadaSeta));
//        this.consequente.add(new TipoToken("(", "PARENTESES_ABRE", linhaEliminadaSeta));
        while (index >= 0) {
        ...
        if (this.consequente.size() > 3) {
            if (!pilha.get(index + 2).getTipo().equals("ESCOPO")) {

                this.consequente.add(0, new TipoToken("(", "PARENTESES_ABRE", linhaEliminadaSeta));
                this.consequente.add(new TipoToken(")", "PARENTESES_FECHA", linhaEliminadaSeta));

            }

        }
        this.consequente.add(0, new TipoToken("~", "NEGACAO", linhaEliminadaSeta));
    ...
    public boolean temIgual() {
        ...
        for (TipoToken consequente1 : this.consequente) {
        ...
        for (TipoToken token : tokens) {
            ...
            if (token.getLinha() == i) {
                ...
                if (tokensLinha.equalsIgnoreCase(consequenteString)) {
                    ...
                    this.regras.add(linhaEliminadaSeta + "," + i + "     Modus Tolens");
    ...
    public void aplicaRegra() {
        ...
        while (!this.precedente.isEmpty()) {
            this.tokens.add(new TipoToken(
                    this.precedente.get(0).getNome(),
                    this.precedente.get(0).getTipo(),
                    ultimaLinha));
            this.precedente.remove(0);
        }
